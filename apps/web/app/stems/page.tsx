...
export default function StemsPage() {
	...
	const uploadOne = useCallback(
		async (file: File): Promise<string> => {
			...
			if (!modeRes.ok)
				throw new Error(String(modeData.error || "Upload config failed"));
	...
	const onFiles = useCallback(
		async (fileList: FileList | null) => {
			...
			const selected = [...fileList].filter((f) =>
				isRetagInput(f.name, f.type),
			);
			...
			try {
				for (let i = 0; i < selected.length; i++) {
					...
					setProgressNote(
						`Uploading ${i + 1}/${selected.length}: ${file.name}`,
					);
					try {
						...
						if (!res.ok)
							throw new Error(String(data.error || "Could not start separation"));
		...
		<div className="ui-scope min-h-screen">
			<div className="mx-auto max-w-2xl px-5 pt-10 pb-28">
				...
				<p className="mt-1 text-xs leading-relaxed text-muted-foreground">
					Split a track into instrumental and vocals. One pass produces both —
							...
							return (
								<article key={t.id} className="relative pl-5">
									<span className="absolute top-1.5 left-0">
										<StatusDot
											status={
												failed ? "failed" : (job?.status ?? "queued")
											}
										/>
									</span>
